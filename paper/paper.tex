...
genome task ($-0.019$, $[-0.037, -0.001]$). Holding the score fixed and varying
the budget instead, we find that the budget needed to reach a fixed capability
falls with scale ($-0.118$ decades per decade of parameters, $[-0.181, -0.055]$).
...
+0.046]$); six runs gave a clear widening. The point estimate barely moved ---
only the interval did.
\end{abstract}
...
The full taxonomy is given in Supplementary Note~S2; eight load-bearing ones
...
and archived at \doi{10.5281/zenodo.22061974}~\citep{kendiukhov2026code}, a
concept DOI that always resolves to the most recent archived version. Result cards are under
...
The author thanks Pivotal Research for supporting this work and Foresight
Institute for providing the compute on which the experiments were run, and
acknowledges support from the Open Access Publication Fund of the University of
T\"ubingen.

\section*{Funding}

This work was supported by Pivotal Research. Compute was provided by Foresight
Institute. Open access publication was supported by the Open Access Publication
Fund of the University of T\"ubingen.

\section*{Declaration of generative AI use}

This declaration follows the ISCB acceptable-use policy for large language
models, which this journal adopts, and the detailed account it asks for is given
in Supplementary Note~S1.

A large language model (Claude, Anthropic) was used as an assistant under the
author's direction. Its uses map onto the policy's acceptable categories as
follows. It assisted in \textbf{writing and refactoring code} --- the evaluation
harness, the analysis functions and the figure-generation scripts --- all of which
are released in the repository named under Data availability and are covered by
its test suite. It was used \textbf{as an evaluation technique}, to search the
pipeline for inconsistencies and anomalies; several of the measurement defects
reported in Section~\ref{sec:defects} were surfaced that way. It was used to
\textbf{retrieve and verify bibliographic metadata}: every reference here was
resolved programmatically against doi.org or the arXiv API by the released script
\texttt{build\_bib.py}, which fails rather than emit an unresolvable entry, and
each resolved record was checked against its claimed author, title and year. It
also \textbf{helped produce parts of the text} of the manuscript and the \LaTeX{}
sources of its tables and figures.

The author set the scope, structure and claims of the paper, made all scientific
judgements and interpretations, and reviewed and edited every output of the
assistant. The author takes full responsibility for the content of the article,
including the correctness of the released code, the accuracy of the text, and the
verification of every reference. The assistant is not, and is not claimed to be,
an author.
