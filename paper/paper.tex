...
study (Section~\ref{sec:defects}). We ran three seeds over five rungs, with nine
budgets (25--400 steps) in the first seed and seven (50--400) in the other two,
and evaluated three target scores. A fourth target of 0.55 was run at the first
...
Every claim maps to a script and a data directory (Table~\ref{tab:repro}), in the
repository named under Data availability. Result cards live under
\texttt{data/cards/seed0}--\texttt{seed5}, one directory per repeat, and the
iso-performance sweeps under \texttt{data/sweeps/}. Reproducing every number needs
neither a GPU nor model weights: the released cards are the input.
...
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{claim} & \textbf{command} \\
\midrule
Tables~\ref{tab:gap}, \ref{tab:families}, Figure~\ref{fig:forest} &
\texttt{python scripts/analyse\_gap.py} \\
Table~\ref{tab:recov} & \texttt{python scripts/analyse\_recoverability.py} \\
Table~\ref{tab:cost}, Figure~\ref{fig:cost} & \texttt{python scripts/analyse\_cost.py} \\
CARP width and depth (Section~\ref{sec:families}) &
\texttt{python scripts/analyse\_carp\_geometry.py} \\
all figures & \texttt{python scripts/make\_figures.py} \\
bibliography & \texttt{python scripts/build\_bib.py} \\
every published number & \texttt{pytest -q} \\
\bottomrule
\end{tabular}
\caption{Every claim in this paper, and the command that reproduces it. The test
suite re-derives each published value from the released cards and fails if it
moves.}
\label{tab:repro}
\end{table}
...
Each budget is a separate fine-tuning run annealed to its own endpoint. Open
triangles mark rungs censored at the grid ceiling: that model never reached the
target inside the largest budget run, so it is excluded from its seed's fit.}
\label{fig:cost}
\end{figure}
...
paper are openly available at
\url{https://github.com/Biodyn-AI/probe-scaling}. Result cards are under
\texttt{data/cards/}; iso-performance sweeps under \texttt{data/sweeps/}. The
analysis scripts, figure-generation script and bibliography generator named in
Section~\ref{sec:repro} regenerate every figure, table and reference from those
files. No new experimental data were generated; all model checkpoints and
...
The scientific claims, their interpretation, and the text of this manuscript are
the author's own. No part of the manuscript was drafted from a prompt. All
outputs of the assistant were reviewed by the author, and every numerical result
reported here was re-derived from the released result cards by the analysis
scripts named in Section~\ref{sec:repro}.
